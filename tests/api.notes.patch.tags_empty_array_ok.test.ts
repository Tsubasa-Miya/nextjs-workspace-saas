...
describe('Notes PATCH - empty tags array ok', () => {
  it('200 when tags is []', async () => {
    process.env.NODE_ENV = 'production';
    vi.resetModules();
    vi.mock('@/src/lib/db', () => ({ prisma: { note: { findUnique: vi.fn(async () => ({ id: 'n1', workspaceId: 'w1' })), update: vi.fn(async () => ({ id: 'n1', tags: [] })) } } }));
